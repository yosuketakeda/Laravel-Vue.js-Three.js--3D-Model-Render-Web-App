        ...
        <div class="col-md-6 form-container mt-3">
            ...
            <div class="sign-up-footer text-center">
                <div class="log-in mb-2">
                    Don't have an account<a href="{{route('suppliers.adminRegister')}}" class="ml-2">Sign Up</a>
                </div>                
            </div>
        </div>
    </div>
...
<div class="modal fade" id="suppliersModal" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header border-0">     
                <h3 class="modal-title">This account is not the supplier admin account.</h3>
            </div>
            <div class="modal-footer border-0">
                <button type="button" class="btn btn-outline-primary rounded-pill" data-dismiss="modal">OK</button>                    
            </div>
...
<script>
    ...
    function adminCheck(){
        ...
        var email = $("#email").val();
        $.ajax({
            url: "{{route('suppliers.adminCheck')}}",
            type: "POST",
            data: {"email": email},
